 ...
 * Modified slightly by Pixelgrade
 *
...
if ( ! function_exists( 'hybrid_media_grabber' ) ) {
	/**
...
if ( ! class_exists( 'Hybrid_Media_Grabber' ) ) {
	...
	class Hybrid_Media_Grabber {
		...
		/**
		 * The actual format of the media to return.
		 *
		 * @since  1.6.1
		 * @access public
		 * @var    array
		...
		public function __construct( $args = array() ) {
			...
			$defaults = array(
				'post_id'       => get_the_ID(),
				// post ID (assumes within The Loop by default)
				'type'          => 'video',
				// audio|video
				'before'        => '',
				// HTML before the output
				'after'         => '',
				// HTML after the output
				'split_media'   => false,
				// Splits the media from the post content
				'width'         => $content_width,
				// Custom width. Defaults to the theme's content width.
				'extra_details' => false,
				// Whether to send a more detailed response, or just the media (this is the default)
			);
			...
			$this->type    = isset( $this->args['type'] ) && in_array( $this->args['type'], array(
				'audio',
				'video',
				'gallery'
			) ) ? $this->args['type'] : 'video';
		...
		public function get_media() {
			if ( true === $this->args['extra_details'] ) {
				return apply_filters( 'hybrid_media_grabber_media', array(
					'content'  => $this->media,
					'original' => $this->original_media,
					'format'   => $this->media_format
				), $this, true );
			}
		...
		public function do_shortcode_media() {
			...
			if ( ! empty( $matches ) ) {
				...
				foreach ( $matches as $shortcode ) {
					...
					elseif ( in_array( $shortcode[2], array(
						'blip.tv',
						'dailymotion',
						'flickr',
						'ted',
		...
		public function do_playlist_shortcode_media( $shortcode ) {
			...
			// Remember where we've found this - the format
			$this->media_format[] = 'playlist_shortcode';
		}
		...
		public function do_embed_shortcode_media( $shortcode ) {
			...
			// Remember where we've found this - the format
			$this->media_format[] = 'embed_shortcode';
		}
		...
		public function do_audio_shortcode_media( $shortcode ) {
			...
			$this->media = do_shortcode( $this->original_media );
			// Remember where we've found this - the format
			$this->media_format[] = 'audio_shortcode';
		}
		...
		public function do_video_shortcode_media( $shortcode ) {
			...
			// Remember where we've found this - the format
			$this->media_format[] = 'video_shortcode';
		}
		...
		public function do_jetpack_shortcode_media( $shortcode ) {
			...
			$this->media = do_shortcode( $this->original_media );
			// Remember where we've found this - the format
			$this->media_format[] = 'jetpack_shortcode';
			// Find the first non-empty entry in $shortcode
			// This should generally be the name of the shortcode
			foreach ( $shortcode as $part ) {
				if ( ! empty( $part ) ) {
					$this->media_format[] = $part . '_shortcode';
					break;
				}
			}

			// This is an external service so it is probably and embed
			$this->media_format[] = 'embed';
		}
		...
		public function do_gallery_shortcode_media( $shortcode ) {
			...
			$this->media = do_shortcode( $this->original_media );
			// Remember where we've found this - the format
			$this->media_format[] = 'gallery_shortcode';
		}
				...
				foreach ( $matches as $value ) {
					...
					if ( $embed ) {
						...
						if ( preg_match( "/\[{$this->type}\s/", $embed ) ) {
							...
							$this->media_format[] = 'autoembed_shortcode';
						}

						$this->original_media = $value[0];
						$this->media          = $embed;
						// Remember where we've found this - the format
						$this->media_format[] = 'autoembed';
						$this->media_format[] = 'embed';
		...
		public function filter_dimensions( $html ) {
			...
			// If no dimensions are found or the dimensions are not numeric (maybe procentual), just return the HTML.
			if ( empty( $media_atts ) || ! isset( $media_atts['width'] ) || ! isset( $media_atts['height'] ) || ! is_numeric( $media_atts['width'] ) || ! is_numeric( $media_atts['height'] ) ) {
				return $html;
			}
